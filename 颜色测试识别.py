...
def color_(img):
	...
	while True:
		...
		#print(len(white_array))		#50
		#print(len(white_array[0]))	#200
		# img_mask获得原图中的指定颜色图像
		#all_img_mask = np.vstack([white_array, img, img_mask])
		#all_img_mask = np.vstack([img, img_mask])
		all_img_mask = np.vstack([img_mask])
...
if __name__ == '__main__':
	path = "../dataset/imgs/pai/du1.png"
	img = cv2.imread(path)
	Lower = np.array([0, 0, 0])  # 要识别颜色的下限
	Upper = np.array([179, 255, 255])  # 要识别的颜色的上限
	#white_array = np.zeros((50,200,3),np.uint8)
	white_array = np.zeros((len(img),len(img[0]),3),np.uint8)
	#white_array.fill(255)
	#img = cv2.resize(img,(200,200))
	color_(img)

#上kdj：lower=[109,0,0] high=[179,255,255]，下macd：lower=[0,0,0] high = [179,64,255]